...
const ParticularTask = ({data}) => {
  ...
  return (
...

    {
        data.map((elem, index)=>{
            return(
                <div key={index} className=' relative flex-1 flex flex-col justify-start  h-fit max-lg:min-w-[250px] lg:min-w-[600px] bg-slate-800/60 hover:shadow-2xl hover:shadow-slate-800 hover:scale-[1.01] transition-all ease-in-out cursor-pointer rounded-lg px-6 py-5'>
                        ...
                        <p className='text-xl font-light leading-tight tracking-wide mt-4'>{elem.description}</p>
                            ...
                            <button className=' absolute bottom-2 right-4  px-4 py-1 bg-green-400 rounded-md font-semibold text-lg'>
